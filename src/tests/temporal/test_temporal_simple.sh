...
# Configuration
SERVER_URL="http://localhost:8085"  # Make sure this matches the actual server URL
...
if [[ "$HISTORY_RESPONSE" == *"history"* ]]; then
  ...
  echo "Entity has $VERSION_COUNT versions"
  
  if [ "$VERSION_COUNT" -eq 2 ]; then
    echo "TEMPORAL HISTORY TEST PASSED!"
  else
    echo "TEMPORAL HISTORY TEST FAILED! Expected 2 versions, got $VERSION_COUNT"
  fi
else
