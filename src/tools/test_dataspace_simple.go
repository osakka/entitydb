...
func main() {
	if len(os.Args) < 2 {
		log.Fatal("Usage: test_dataspace_simple <data_directory>")
	}
	...
	temporalRepo, err := binary.NewTemporalRepository(dataDir)
	if err != nil {
		log.Fatalf("Failed to create temporal repository: %v", err)
	}
	...
	relRepo, err := binary.NewRelationshipRepository(repo)
	if err != nil {
		log.Fatalf("Failed to create relationship repository: %v", err)
	}
	...
	canAccessRel := &models.EntityRelationship{
		...
		Properties:   map[string]string{"created": models.NowString()},
	}
